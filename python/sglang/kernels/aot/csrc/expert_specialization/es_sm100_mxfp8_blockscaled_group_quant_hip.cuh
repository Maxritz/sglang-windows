...
__inline__ __device__ uint8_t cvt_warp_fp16_to_mxfp8(FragmentS& fragment_s, FragmentD& fragment_d) {
  ...
  using VecType = std::conditional_t<std::is_same_v<ValType, __hip_bfloat16>, __hip_bfloat162, __half2>;
  VecType vec[8];
  // Assign vals
  ...
  if constexpr (std::is_same_v<ValType, __hip_bfloat16>) {
...
void launch_es_sm100_mxfp8_blockscaled_grouped_quant(
  ...
  AT_CUDA_CHECK(hipOccupancyMaxActiveBlocksPerMultiprocessor(
 ...
 hipLaunchKernelGGL(( mxfp8_group_quant<T_IN, decltype(tiled_copy_g2r), decltype(tiled_copy_r2g), decltype(tiled_copy_r2s)>)
