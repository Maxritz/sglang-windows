...
#pragma unroll
    for (uint32_t i = 0; i < vec_size; ++i) {
      out_vec[i] = castFromFloat<T>(castToFloat(Activation(x_vec[i])) * castToFloat(y_vec[i]));
    }
...
#pragma unroll 1
  for (int64_t idx = thread_idx; idx < d % (stride * vec_size); idx += stride) {
    ...
    out[token_idx * d + remaining_offset + idx] = castFromFloat<T>(castToFloat(Activation(x)) * castToFloat(y));
  }
}
